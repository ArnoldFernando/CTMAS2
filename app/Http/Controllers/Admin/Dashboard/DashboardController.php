...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $weeklyAverageData = $this->calculateWeeklyAverage();

        // Extract the weekly count and average from the result
        $weeklyCount = $weeklyAverageData['weeklyCount'];
        $weeklyAverage = $weeklyAverageData['weeklyAverage'];
        ...
        return view('admin.dashboard', compact('dailyCount', 'weeklyCount', 'weeklyAverage', 'monthlyTotal', 'courseCount', 'averageTimePerCourse'));
    }
    ...
    private function calculateWeeklyAverage()
    {
        ...
        // $startOfSecondWeek = Carbon::create(2024, 10, 8)->startOfDay(); // October 8
        // $endOfSecondWeek = Carbon::create(2024, 10, 14)->endOfDay();   // October 14

        // Fetch the count of records with time_in within this week
        $weeklyCount = DB::table('student_records')
            ->whereBetween('created_at', [$startOfWeek, $endOfWeek])
            ->count();

        // Calculate the average count for this week
        $daysInWeek = 7; // Standard week length
        $weeklyAverage = $weeklyCount / $daysInWeek;
